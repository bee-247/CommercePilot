...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/api/rag/auth/login")


def _validated_secret_key() -> str:
    if len(SECRET_KEY) < 32:
        raise RuntimeError("JWT_SECRET_KEY 必须配置为至少 32 个字符")
    return SECRET_KEY
...
def create_access_token(username: str, role: str) -> str:
    ...
    return jwt.encode(payload, _validated_secret_key(), algorithm=ALGORITHM)
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> User:
    ...
    try:
        payload = jwt.decode(token, _validated_secret_key(), algorithms=[ALGORITHM])
...
def ensure_bootstrap_admin() -> None:
    """Create or promote the configured bootstrap administrator idempotently."""
