...
from rag.storage.models import User, ChatSession, ChatMessage
...
def _format_multi_task_response(parts: list[tuple[ServiceSubTask, str]]) -> str:
...
def _route_summary(tasks: list[ServiceSubTask]) -> str:
    return " -> ".join(task.route for task in tasks)


def _format_task_plan_markdown(tasks: list[ServiceSubTask]) -> str:
...
def _task_plan_payload(tasks: list[ServiceSubTask]) -> list[dict]:
    return [
...
def _log_task_plan(tasks: list[ServiceSubTask]) -> None:
...
def chat_with_agent(user_text: str, user_id: str = "default_user", session_id: str = "default_session"):
    ...
    try:
        ...
        reset_service_username(context_token)
