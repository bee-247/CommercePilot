    ...
    def init_collection(self, dense_dim: int | None = None):
        ...
        def _init(client: MilvusClient) -> None:
            if not client.has_collection(self.collection_name):
                ...
                schema.add_field("resource_id", DataType.INT64)
                ...
                schema.add_field("business_line", DataType.VARCHAR, max_length=100)
                schema.add_field("document_type", DataType.VARCHAR, max_length=50)
                schema.add_field("section_title", DataType.VARCHAR, max_length=255)
                schema.add_field("product_tags", DataType.JSON)
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    "id": hit.get("id"),
                    "text": hit.get("text", ""),
                    "resource_id": hit.get("resource_id", 0),
                    "owner_id": hit.get("owner_id", 0),
                    ...
                    "product_tags": hit.get("product_tags", []),
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    ...
                    "resource_id": hit.get("entity", {}).get("resource_id", 0),
                    "owner_id": hit.get("entity", {}).get("owner_id", 0),
                    "visibility": hit.get("entity", {}).get("visibility", "public"),
                    ...
                    "business_line": hit.get("entity", {}).get("business_line", ""),
                    "document_type": hit.get("entity", {}).get("document_type", ""),
                    "section_title": hit.get("entity", {}).get("section_title", ""),
                    "product_tags": hit.get("entity", {}).get("product_tags", []),
