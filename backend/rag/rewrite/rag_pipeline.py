...
from ..retrieval.rag_utils import retrieve_documents, step_back_expand, generate_hypothetical_document
from ..utils.tools import emit_rag_step
...
def retrieve_initial(state: RAGState) -> RAGState:
    ...
    rag_trace = {
        "tool_used": True,
        "tool_name": "search_customer_knowledge",
        "query": query,
...
def run_rag_graph(question: str) -> dict:
    ...
    state.update(retrieve_initial(state))
    state.update(grade_documents_node(state))
    if state.get("route") == "rewrite_question":
        state.update(rewrite_question_node(state))
        state.update(retrieve_expanded(state))
    return state
