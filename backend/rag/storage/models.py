...
class User(Base):
    ...
    service_artifacts = relationship(
        "ServiceArtifact",
...
class Resource(Base):
    ...
    business_line: Mapped[str] = mapped_column(String(100), default="", nullable=False)
    document_type: Mapped[str] = mapped_column(String(50), default="product", nullable=False)
...
class ParentChunk(Base):
    ...
    product_tags: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
...
class ServiceArtifact(Base):
    ...
    owner = relationship("User", back_populates="service_artifacts")
